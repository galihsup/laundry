...
$saldo_debet = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT SUM(jurnal.saldo_jurnal) AS debet FROM jurnal JOIN coa ON jurnal.coa_id = coa.no_coa WHERE DATE_FORMAT(tanggal, '%Y-%m-%d') < '$t-$b-01' AND coa.gol = 'D'"));
$saldo_kredit = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT SUM(jurnal.saldo_jurnal) AS kredit FROM jurnal JOIN coa ON jurnal.coa_id = coa.no_coa WHERE DATE_FORMAT(tanggal, '%Y-%m-%d') < '$t-$b-01' AND coa.gol = 'C'"));
                        ...
                        <tbody>
                            ...
                            <?php while($row = mysqli_fetch_assoc($query)): ?>
                                <tr>
                                    ...
                                    <td>Rp. <?= ($row['gol'] == 'K') ? number_format($row['saldo_jurnal'], 0, ',', '.') : '' ?></td>
