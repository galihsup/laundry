...
$sql_transaksi = "INSERT INTO transaksi (kode_transaksi, pemesan, telp, alamat, logistik, diskon, total, tunai, id_member, debit, credit) VALUES ('$kode_transaksi', '$nama_pemesan', '$telp', '$alamat', '$logistik', '$diskon', '$total', '$tunai', '$id_member', '11101', '41101')";
...
if ($sql_transaksi) {
    ...
    $sql_jurnal = "INSERT INTO jurnal (kode_jurnal, tanggal, saldo_jurnal, coa_id) VALUES ('$kode_jurnal', '$tanggal', '$total', '11101')";
    mysqli_query($koneksi, $sql_jurnal);

    $sql_get_data_kas = "SELECT saldo from coa where no_coa = '11101'";
    $saldo_kas = mysqli_fetch_assoc(mysqli_query($koneksi, $sql_get_data_kas));
    $saldo_kas_now = $saldo_kas['saldo'] + $total;
    $sql_update_saldo_kas = "UPDATE coa set saldo = '$saldo_kas_now' WHERE no_coa ='11101'";
    mysqli_query($koneksi, $sql_update_saldo_kas);

    $sql_get_data_penus = "SELECT saldo from coa where no_coa = '41101'";
    $saldo_penus = mysqli_fetch_assoc(mysqli_query($koneksi, $sql_get_data_penus));
    $saldo_penus_now = $saldo_penus['saldo'] + $total;
    $sql_update_saldo_penus = "UPDATE coa set saldo = '$saldo_kas_now' WHERE no_coa ='41101'";
    mysqli_query($koneksi, $sql_update_saldo_penus);
